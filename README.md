...
- `convex/schema.ts` defines the domain schema: staff profiles (admin/operator/viewer roles), reporting operations, clinic groups, clinics, clinic column mappings, reporting scopes, QA groups, and report runs.
...
- `convex/staffAccounts.ts` + `convex/model/staff.ts` implement staff account management (roles, status, admin-guarded queries).
