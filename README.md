...
- `convex/schema.ts` defines the domain schema: staff profiles, clients, clinics (with embedded `sheetColumns` and `qaGroupKeys`), and report runs. Report operations are a fixed list in `convex/model/reportOperations.ts`, not a table.
